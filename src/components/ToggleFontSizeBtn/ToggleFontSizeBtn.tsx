...
export const ToggleFontSizeBtn = () => {
  const increaseText = () => {
    document
      .querySelectorAll('.leading-8, .descParagrah, .summaryCard div p')
      .forEach((item) => {
  ...
  return (
    <div className="flex flex-row gap-4 items-center  hover:cursor-pointer">
      <span onClick={increaseText} className="text-[0.95rem]">
        A+
      </span>
      <span onClick={decreaseText} className="text-[0.95rem]">
        A-
      </span>
    </div>
  );
