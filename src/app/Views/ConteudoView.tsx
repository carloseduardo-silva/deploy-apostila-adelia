...
import type { ClientViewProps, ContentBlock } from "../types/MaterialTypes";
...
function fixContentHtml(html: string): string {
  const baseUrl = "https://apimaterial.sistemaeditoracapro.com.br";
  return html.replace(/http:\/\/147\.93\.9\.183:1337/g, baseUrl);
}

export default function ContentView({ apostila, tag }: ClientViewProps) {
  ...
  function renderContentItem(item: ContentBlock) {
    ...
    return (
      ...
      >
        {type === "Codigo" ? (
          <>
            <h2 className="text-left mb-2 text-[23px] md:text-2xl font-bold uppercase text-red-700">
              {title}
            </h2>
          ...
          <>
            <h2 className="titleMaterial nunito text-[23px] text-left md:text-[29px] mb-2 font-bold text-red-600">
              {title}
            </h2>
            <div
              className="leading-8 text-[18px] text-justify text-gray-900"
              dangerouslySetInnerHTML={{ __html: fixContentHtml(body_text) }}
            />
          </>
        )}
      </div>
  ...
  return (
    <>
      <main className="flex h-full w-full flex-col min-h-[100vh]">
        ...
        <div className="flex-1 my-0 mx-auto flex w-full flex-col pb-[3.5rem] md:px-[3rem] max-w-[2000px]">
          <div className="flex flex-col md:flex-row">
            ...
            <div className="flex-1 w-full md:w-[90%] mx-auto">
              <div className="flex flex-1 items-start justify-center text-center text-[18px] md:text-[20px] px-4 mb-5">
                <p
                  className="descParagrah"
                  style={{ color: "rgb(124, 122, 128)" }}
                >
                  Leia, pense e reflita sobre o conteúdo desta unidade. Se
                  necessário, procure ajuda do professor/tutor.
                </p>
              </div>
              <div className="flex flex-col gap-[4rem] pb-[2rem] pt-[1rem] px-4 md:px-12 text-[18px] leading-8 text-justify">
                {apostila?.content_blocks?.map((item) => (
        ...
        <MenusConteudo tag={tag} hiddenCard="apostila" />
